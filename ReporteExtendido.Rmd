...
fn <- function(dt, n) {
  nn <- c()
  for (i in seq(1,n)) {
    nn <- c(nn, is.numeric(dt[1,i]))
  }
  nn
}

data2 <- ex_GP[,fn(ex_GP, 23)]
```
...
- __1: __¿Es posible predecir la cantidad de descargas de una aplicación, en base a los datos disponibles en Google Play?¿Se puede hacer la misma preducción con los datos de las reviews?

- __2: __¿Es posible caracterizar tipos de aplicaciones respecto a los reviews recibidos por los usuarios? 

- __3: __¿Existen características específicas de las aplicaciones que permitan tener mejor o peor aprobación del público?¿Es posible predecir el rating?
...
pex_GP <- pex_GP[pex_GP$Rating > 2.5,]
...
travel <- travel[travel$Installs<5000000,]
tools <- tools[tools$Installs<50000000,]
social <- social[social$Installs<5000000,]
productivity <- productivity[productivity$Installs<50000000,]
photogrphy <- photogrphy[photogrphy$Installs<500000,]
magazines <- magazines[magazines$Installs<5000000,]
...
clusdt <- clusdt[clusdt$Rating > 2.5,]
...
Se considera el rating, número de reseñas, peso, publico objetivo y si es de pago o no para clasificar las descagas según cuartiles (pregunta 1). Se clasifica el rating según cuartiles con los mismo atributos, reemplazando "Rating" por "Installs" (Pregunta 3). No se concidera el género debido a que pueden existir combinaciones categoría/género no contempladas en los datos.
