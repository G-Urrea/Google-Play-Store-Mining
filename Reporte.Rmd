...
addQD <-function(x) {
  x <- merge(x, data.frame(App=x$App, Rating_Q=markQuartile(x$Rating), Installs_D=markDecile(x$Installs)), by="App")
}
...
Se seleccionan las columnas "Rating", "Reviews", "Size", "Type" y "Content.Rating" para clasificar las descagas según deciles (pregunta 1). Se clasifica el rating según cuartiles con los mismo atributos, reemplazando "Rating" por "Installs" (Pregunta 3). No se concidera el género debido a que pueden existir combinaciones categoría/género no contempladas en los datos.
...
Para todas las clasificaciones mencionadas se utlizan los modelos de árbol de decisión, KNN y naive bayes. Para evaluar los resultados de la clasificación se hará uso de cross validation con distribución 70/30 entre datos de entrenamiento y prueba, y considerarán las métricas: exactitud, error, precisión, recall y F1, se considerará también una curva ROC.

### Clustering

Sobre estos datos se aplican los siguentes métodos: _K_-means, con valores de _k_ obtenidos según optimos observables en un gráfico de codo; clustering jerárquico aglomerativo, con criterios _complete_, _average_, _single_ y _ward_; y DBSCAN con valores de _eps_ y _minPts_ seleccionados según __[inserte método de selección]__.

Se busca caracterizar a las aplicaciones a través de sus reviews, lo cual se puede interpretar de dos maneras. La primera es a nivel "individual", en el sentido que es independiente clasificaciones previas de las aplicaciones como género o categoría, y se centra integramente en caracterizar la aplicación según las reseñas que recibe, esperando obtener resultados del tipo "la aplicación _A_ recibe comentarios del tipo _B_". Para este caso se podrían aplicar más filtros y dividir los datos en subconjuntos más homogéneos (según número de descargas, categorías, content rating, ect.) sin embargo, el dataset actual ya es demasiado pequeño (607 filas).

Los resultados se evaluan en base a cohesión (SSE), separació (BSS) y coeficiente de Silhouette. Para visualizar los resultados se reduce la dimensionalidad con el método _principal component analisys_, de modo que sea posible crear un gráfico de disperción. También se estudia la distribución de loss atributos en los clusters resultantes, en específico las medias y cuartiles.

La segunda opción es evaluar la posibilidad de que, por medio de sus reviews, se logre caracterizar las aplicaciones según datos externos a esta, como lo son la categoría o el rating. En este caso se evaluan los resultados comprobando las diferencias y similitudes de las aplicaciones de los clusters resultantes, según sus atributos en el dataframe 'Apps'. __[Insertar método para evualuar similitud y diferencia]__
